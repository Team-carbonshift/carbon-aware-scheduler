...
N_DAYS = 365                # 1년 통일 (개발용 축소 시에만 줄일 것)
START_DATE = "2026-01-01"   # t=0 (UTC), 기록용
OUT_PATH = str(__import__("pathlib").Path(__file__).resolve().parent.parent / "jobs.csv")  # → 03_데이터/jobs.csv
...
K_TABLE = {
    5: dict(w=3, l_rng=(1, 5),          l_fix=1,     dur_med=5,    dur_sig=0.6, clamp=(1, 60)),
    4: dict(w=2, l_rng=(15, 60),        l_fix=30,    dur_med=30,   dur_sig=0.7, clamp=(1, 300)),
    3: dict(w=1, l_rng=(120, 600),      l_fix=300,   dur_med=180,  dur_sig=0.8, clamp=(5, 1800)),
    2: dict(w=1, l_rng=(7200, 28800),   l_fix=21600, dur_med=1800, dur_sig=0.9, clamp=(60, 14400)),
    1: dict(w=3, l_rng=(43200, 86400),  l_fix=86400, dur_med=5400, dur_sig=0.9, clamp=(300, 21600)),
}
DIURNAL_K = {5, 4, 3}
